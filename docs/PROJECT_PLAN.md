...
- 添加 prompt 模板，作为未来 LLM 讲解和核查的约束。
- 先实现不依赖 API key 的证据抽取版报告生成。
- 分章节整理摘要、方法、数学、实验、结果、局限性和可复现性线索。
- 明确标记数学、方法和实验部分的缺失证据，不补写没有材料依据的推导或结论。
- 将最终报告保存为 Markdown。
...
- 询问讲解质量评分。
- 询问可选标签或备注。
- 保存反馈到 `data/history/feedback.jsonl`。
- 根据全部反馈更新 `data/history/profile.json`。
- 在后续推荐时打印轻量偏好提示。
